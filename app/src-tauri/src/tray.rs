...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/mindflow.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("MindFlow v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("MindFlow v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
pub fn update_tray_menu(app: &AppHandle, state: &TrayIconState, locale: Option<&str>) {
    ...
    let mut downloaded: Vec<_> = models
        .into_iter()
        .filter(|m| {
            m.is_downloaded && !matches!(m.engine_type, crate::managers::model::EngineType::TextLlm)
        })
        .collect();
