  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2 text-xs text-text-secondary min-w-0">
          <span
            className={`inline-block w-2 h-2 rounded-full shrink-0 ${
              paused ? "bg-text-secondary" : "bg-emerald-500 animate-pulse"
            }`}
          />
      ...
      <div
        ...
        className="h-72 overflow-y-auto rounded-lg border border-border bg-[var(--color-log-surface)] p-3 font-mono text-xs leading-relaxed select-text"
      >
        {logs.length === 0 ? (
          <div className="text-text-secondary select-none">
          ...
          logs.map((line) => {
            ...
            return (
              <div key={line.id} className="flex gap-2">
                <span className="text-text-secondary/80 shrink-0 select-none tabular-nums">
                  {line.time}
                </span>
                <span
